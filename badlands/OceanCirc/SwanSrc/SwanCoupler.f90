module swan_coupler_parallel
    ...
    ! SWAN model decomposition for Badlands
    integer :: Xsdiv, Ysdiv
    type badlands_WADecomp
        integer :: X_nb
        integer :: Y_nb
        integer :: minX_id
        integer :: minY_id
...
contains
    ...
    subroutine export_swan( MIP, MXK, MYK, NVOQP, VOQ, VOQR, IONOD )
        ...
        m = 1
        n = 1
        call mpi_barrier(mpi_comm_world,inerr)
...
!                    exportSWANfields( m, n, 3 ) = real( VOQ( id+ xik, VOQR(IV3)) )
!                    exportSWANfields( m, n, 4 ) = real( VOQ( id+ xik, VOQR(IV4)) )
!                    exportSWANfields( m, n, 5 ) = real( VOQ( id+ xik, VOQR(IV5)) )
                    m = m + 1
                    if( m > Isize )then
                        m = 1
                        n = n + 1
...
module swan_coupler_functions
    ...
    type ImpSpecWa
        ! [hs] the signi�cant wave height (in m).
        real :: hs
...
contains
    ...
    subroutine swan_decomposition_grid( nbPet, SWANdecomp )
        ...
        type( badlands_WADecomp ), dimension( nbPet ) :: SWANdecomp
        ...
        call mpi_allgather( nbX, 1, SWINT, nbXT, 1, SWINT, mpi_comm_world, inerr )
        call mpi_allgather( nbY, 1, SWINT, nbYT, 1, SWINT, mpi_comm_world, inerr )
        call mpi_allgather( minX, 1, SWINT, minXT, 1, SWINT, mpi_comm_world, inerr )
        call mpi_allgather( minY, 1, SWINT, minYT, 1, SWINT, mpi_comm_world, inerr )
        call mpi_allgather( maxX, 1, SWINT, maxXT, 1, SWINT, mpi_comm_world, inerr )
        call mpi_allgather( maxY, 1, SWINT, maxYT, 1, SWINT, mpi_comm_world, inerr )
        ...
        allocate( exportSWANfields( nbX, nbY, 2 ) )
