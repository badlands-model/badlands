...
def output_cellsIDs(lGIDs, inIDs, visXlim, visYlim, coords, cells):
    ...
    notBorder = numpy.where(
        (coords[lGIDs, 0] >= visXlim[0])
        & (coords[lGIDs, 0] <= visXlim[1])
        & (coords[lGIDs, 1] >= visYlim[0])
        & (coords[lGIDs, 1] <= visYlim[1])
    )[0]
...
def write_xmf(
    ...
    if rockOn:
        ...
        if nbSed > 1:
            f.write(
                '         <Attribute Type="Scalar" Center="Node" Name="depSpecies1">\n'
            )
            f.write(
                '          <DataItem Format="HDF" NumberType="Float" Precision="4" '
            )
            f.write(
                'Dimensions="%d 1">%s:/depSpecies1</DataItem>\n' % (nodes[p], pfile)
            )
            f.write("         </Attribute>\n")

        if nbSed > 2:
            f.write(
                '         <Attribute Type="Scalar" Center="Node" Name="depSpecies2">\n'
            )
            f.write(
                '          <DataItem Format="HDF" NumberType="Float" Precision="4" '
            )
