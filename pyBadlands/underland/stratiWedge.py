...
class stratiWedge():
    ...
    layerElev
        Numpy array containing each layer elevation in the recorded stratigraphic column
    """

    def __init__(self, layNb, elay, xyTIN, folder, h5file,
        ...
        # self.paleoDepth = numpy.zeros([self.ptsNb,layNb+eroLay+1])
        # # Deposition thickness for each type of sediment
        # self.depoThick = numpy.zeros([self.ptsNb,layNb+eroLay+1,sedtype])
        # # Erodibility coefficients for each type of sediment
        # self.Ke = numpy.zeros([self.ptsNb,layNb+eroLay+1,sedtype])
        #
        ...
        #         if thickMap[l] == None:
        #             self.depoThick[:,elay+1-l,sed]  = thickVal[l]
        #         # Thickness map
        #         else:
        #             tMap = pandas.read_csv(str(thickMap[l]), sep=r'\s+', engine='c',
        #                                header=None, na_filter=False, dtype=numpy.float, low_memory=False)
        #             rtMap = numpy.reshape(tMap.values,(len(regX), len(regY)), order='F')
        #             self.depoThick[bPts:,elay+1-l,sed] = interpolate.interpn( (regX, regY), rtMap, xyTIN[bPts:,:], method='linear')
        #             # Assign boundary nodes
        #             tmpH = self.depoThick[bPts:,elay+1-l]
        #             self.depoThick[:bPts,elay+1-l] = tmpH[inID]
        #
        #         # Uniform erodibility value
        #         if eroMap[l] == None:
        #             self.Ke[:,elay+1-l,sed] = eroVal[l]
        #         # Erodibility map
        #         else:
        #             eMap = pandas.read_csv(str(eroMap[l]), sep=r'\s+', engine='c',
        #                                header=None, na_filter=False, dtype=numpy.float, low_memory=False)
        #             reMap = numpy.reshape(eMap.values,(len(regX), len(regY)), order='F')
        #             self.Ke[bPts:,elay+1-l,sed] = interpolate.interpn( (regX, regY), reMap, xyTIN[bPts:,:], method='nearest')
        #             # Assign boundary nodes
        #             tmpK = self.Ke[bPts:,elay+1-l]
        #             self.Ke[:bPts,elay+1-l,sed] = tmpK[inID]
        #
        #     self.depoThick[:,0] = 1.e6
        #     self.Ke[:,0,:] = self.Ke[:,1,:]
        # else:
        #
        #
